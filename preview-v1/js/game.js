(function(){
  ...
  // música já vem cortada certinha (começa em 0:00 = o antigo 1:43) — toca do
  // zero sem precisar pular pra lugar nenhum, então não sofre mais dos bugs de seek.
  const CLIMAX_AT = 24;         // clímax exato dentro do arquivo já cortado
  const WAIT_DURATION = CLIMAX_AT; // ~24s de logos antes do clímax
  const BG_REVEAL_DELAY = 700;  // ms — o "fundo" da logo entra este tempo depois das letras
  const PRESS_START_DELAY = 2400; // ms depois do clímax até o botão aparecer

  // 3 logos fictícias, comprimidas nesses ~24s (cada uma com fade-in/hold/fade-out)
  const SPLASH_CARDS = [
  ...
  let introBattleActive = false; // as bolinhas só lutam a partir do clímax (nome do jogo)

  /* ============ CANVAS: simulação de guerra + brasa + feixe de luz ============ */
  ...
  function skipToClimax(){
    ...
    clockStart = performance.now() - WAIT_DURATION*1000; // realinha o relógio pro instante do clímax
    tryPlayAudio(); // conta como gesto do usuário, libera o áudio se ainda não tinha
  ...
  // volume base (0-1) ajustável pelo slider a qualquer momento
  let userVolume = 0.7;
  if(volSlider){
  ...
  // relógio próprio (não depende do áudio) — a intro roda automaticamente ao carregar a página
  ...
  // ignorado, reset sozinho pro 0:00, etc). A Web Audio API agenda a reprodução
  // de um jeito preciso e não sofre desses bugs — uma vez iniciado no ponto
  ...
  // usado quando a partida de verdade começa — a música do lobby não faz sentido continuar
  // tocando durante a batalha, então some com um fade curtinho em vez de cortar seco.
...
const CHAMPION_PORTRAITS = {
  gelida: 'data:image/jpeg;base64,',
  raio: 'data:image/jpeg;base64,',
...
const CHAMPION_CATALOG = {
  ...
  shava:{name:'Shava',element:'vento',role:'Lutadora de Capoeira',cost:120,hp:140,atk:16,range:1,speed:1.3,special:'shava',desc:'Quando um inimigo está a 3+ blocos, avança até ele (dano dobrado), priorizando quem tem mais vida — recarrega em 10s e só ativa com o alvo a até 5 blocos. Se o alvo estiver abaixo de 10% de vida, executa na hora e arremessa o corpo em outro inimigo próximo (atordoa 2s, reduz dano dele por 5s). Senão, empurra o alvo até 5 blocos — se bater em parede/obstáculo, atordoa 4s e reduz armadura por 3s. A cada 3 golpes, cura 10% do dano causado. A cada 6 golpes no mesmo alvo, cria um vórtice que atordoa (0,8s, sem dano) inimigos num raio de 2 blocos, exceto o próprio alvo.'},
  ...
  jedegar:{name:'Jedegar',element:'terra',role:'Pilar da Perseverança',cost:130,hp:190,atk:10,range:1,speed:0.9,special:'jedegar',desc:'A cada 3,1s ergue um pilar de pedra perto do aliado MAIS DISTANTE dela (assim os pilares se espalham pelo time, mesmo se ela avançar sozinha) — pilares amplificam o dano de aliados num raio de 2 blocos em 10% (dobra pra 20% depois do 5º pilar), e cada um aguenta 2 golpes de inimigo comum (1 de chefe) antes de cair. Ao ser atingido, um pilar libera energia que fortalece os pilares próximos por 3s. Ao erguer o 5º pilar da partida, o efeito de todos dobra permanentemente e inimigos perto de pilares perdem 30% de defesa. Na primeira vez que cai abaixo de 30% de vida, ergue um pilar debaixo do inimigo mais próximo (arremessando ele longe) e foge sobre uma fileira de pedra recém-criada. Ao morrer, ergue um círculo protetor ao redor do aliado mais ferido (escudo, dano e velocidade de ataque por 5s) — os pilares dela desmoronam 5s depois da sua morte.'},
  pyra:{name:'Pyra',element:'fogo',role:'Maga',cost:85,hp:100,atk:21,range:3,speed:0.7,special:'explosao',desc:'A cada 3 golpes, sua explosão também atinge o inimigo mais próximo do alvo com dano total. Como todo Stack User de longa distância, causa mais dano quanto mais longe estiver do alvo (+8% por bloco além do 1º).'},
  ...
  zeph:{name:'Zeph',element:'vento',role:'Arqueira veloz',cost:95,hp:80,atk:14,range:3,speed:1.12,special:'chuva',desc:'A cada 4 tiros, dispara uma flecha extra em outro inimigo aleatório. Como todo Stack User de Vento, tem 20% de chance de esquivar de ataques físicos (sobe pra 30% com menos de 40% de vida). Não vale contra habilidades/passivas. Como todo Stack User de longa distância, causa mais dano quanto mais longe estiver do alvo (+8% por bloco além do 1º).'},
  ima:{name:'Ímã',element:'metal',role:'Controladora',cost:100,hp:105,atk:15,range:1,speed:1,special:'ima',desc:'A cada 4 golpes, magnetiza o alvo, puxando-o pra perto dela e causando dano extra. Passiva: aliados de metal adjacentes a ela ganham +8% de defesa.'},
  frosk:{name:'Frosk',element:'gelo',role:'Lutador Congelante',cost:70,hp:115,atk:16,range:1,speed:1.25,special:'congelamento',desc:'Passiva: pele de gelo permanente, reduz todo dano recebido em 15%. A cada 4 golpes, congela o alvo, paralisando por 1,5s — e causa 30% de dano extra nesse golpe.'},
  gelida:{name:'Gélida',element:'gelo',role:'Espectro Glacial',cost:115,hp:130,atk:20,range:1,speed:1,special:'congelamento',desc:'A cada 5 golpes, congela o alvo, paralisando por 0,5s. Passiva: ao morrer, fica em espectro por 5s — mira o inimigo com MENOS vida da arena inteira. Se ele já estiver com 5% de vida ou menos, ela o executa na hora (só demora 3s pra trocar de lugar); senão, arremessa um pilar de gelo nele, jogando-o até a parede, atordoando-o por 3s (imóvel até acabar) e causando 30 de dano a cada 0,5s. Se o alvo morrer antes do espectro acabar, ela toma o lugar dele, volta à batalha com 60% de vida E dobra o próprio dano até o fim da onda. Só pode renascer 2 vezes por partida — depois disso, se morrer de novo, é de vez.'},
  ...
  shecry:{name:'Shecry',element:'gelo',role:'Colosso Glacial',cost:125,hp:245,atk:18,range:1,speed:0.85,special:null,desc:'Quanto mais vida ela perdeu, mais dano ela causa (até +120%); quanto mais vida ela ainda tem, mais resistente ela fica (até +35% de redução de dano). Passiva: ao cair abaixo de 30% de vida (uma vez por batalha), ganha um escudo baseado na vida máxima e cria uma aura que reduz a velocidade de movimento e ataque de quem estiver a até 2 blocos dela — o efeito da aura vai enfraquecendo aos poucos ao longo de 6 segundos.'},
  nerith:{name:'Nerith',element:'agua',role:'Profetisa das Marés',cost:135,hp:300,atk:5,range:1,speed:0.9,special:null,desc:'Bate muito fraco sozinha — sua força de verdade são os tentáculos. Invoca um tentáculo perto dela mesma, num ritmo que cresce a cada um (2,5s, 3s, 3,5s, 4s), até um limite de 4 vivos ao mesmo tempo — se um for destruído, o próximo demora só 1,5s. Os tentáculos não se movem, atacam com alcance de 3 blocos, causam dano considerável, mas têm pouca vida — fáceis de destruir. Se Nerith morrer, todos os tentáculos vivos entram em frenesi: atacam mais rápido e miram inimigos aleatórios até serem destruídos.'},
...
const ITEM_CATALOG = {
  ...
  punho_serra:{name:'Punho de Serra', kind:'corpo a corpo', cost:70, desc:'Ignora 15% da redução de dano do alvo a cada golpe, e dá +40 de HP — só corpo a corpo (alcance 1). Ótimo contra tanques.', effect:{hpFlat:40, appliesTo:'melee'}},
  presas_sangrentas:{name:'Presas Sangrentas', kind:'corpo a corpo', cost:75, desc:'Cura 8% do dano causado como vida a cada golpe, e dá +35 de HP — só corpo a corpo (alcance 1). Vampirismo bem mais forte que o Amuleto Vital, mas exclusivo de quem luta de perto.', effect:{hpFlat:35, appliesTo:'melee'}},
  furia_crescente:{name:'Fúria Crescente', kind:'corpo a corpo', cost:70, desc:'18% de chance a cada golpe de causar 50% de dano bônus — só corpo a corpo (alcance 1).', effect:{appliesTo:'melee'}},
  botas_lutador:{name:'Botas de Lutador', kind:'corpo a corpo', cost:65, desc:'+22% de velocidade de ataque — só corpo a corpo (alcance 1). Mais forte que as Botas Aceleradas normais, mas exclusiva de quem luta de perto.', effect:{speedPct:0.22, appliesTo:'melee'}},
  ...
  carniceiro:{name:'Carniceiro', kind:'combinado · corpo a corpo', recipe:['punho_serra','presas_sangrentas'], desc:'Combinação de Punho de Serra + Presas Sangrentas. Ignora 25% da redução de dano do alvo, cura 12% do dano causado como vida, e dá +65 de HP — só corpo a corpo.', effect:{hpFlat:65, appliesTo:'melee'}},
  fio_mortal:{name:'Fio Mortal', kind:'combinado · corpo a corpo', recipe:['manopla','punho_serra'], desc:'Combinação de Manopla de Aço + Punho de Serra. +35% de dano, ignora 25% da redução de dano do alvo, e dá +45 de HP — só corpo a corpo. Build de dano com um pouco de fôlego.', effect:{atkPct:0.35, hpFlat:45, appliesTo:'melee'}},
  investida_feroz:{name:'Investida Feroz', kind:'combinado · corpo a corpo', recipe:['manopla','botas_lutador'], desc:'Combinação de Manopla de Aço + Botas de Lutador. +35% de dano e +22% de velocidade de ataque — só corpo a corpo. Build agressiva, ótima pra quem depende de acertar vários golpes rápido pra ativar a habilidade (Raio, Frosk, Nyx).', effect:{atkPct:0.35, speedPct:0.22, appliesTo:'melee'}},
  olho_falcao:{name:'Olho de Falcão', kind:'combinado · longa distância', recipe:['luneta','botas'], desc:'Combinação de Luneta de Longo Alcance + Botas Aceleradas. +1 de alcance e +15% de dano — mas o efeito de verdade é a mira: em vez de atacar o inimigo mais próximo, sempre foca automaticamente em quem estiver com MENOS vida dentro do alcance. Só alcance 2+.', effect:{range:1, atkPct:0.15, appliesTo:'ranged'}},
  ...
  precisao_mortal:{name:'Precisão Mortal', kind:'combinado · longa distância', recipe:['mira','luneta'], desc:'Combinação de Mira Telescópica + Luneta de Longo Alcance. +2 de alcance, +30% de dano e +30% de velocidade de ataque — só alcance 2+.', effect:{range:2, atkPct:0.30, speedPct:0.30, appliesTo:'ranged'}},
  brasa_eterna:{name:'Brasa Eterna', kind:'combinado · dano ativo', recipe:['brasa','amplificador'], desc:'Combinação de Brasa Selvagem + Núcleo Amplificador. A Brasa Selvagem ativa 30% mais rápido (a cada 3,5s) e causa 75% mais dano — o efeito elemental de cada personagem continua o mesmo.', effect:{}},
  vigor_absoluto:{name:'Vigor Absoluto', kind:'combinado · vida', recipe:['manto_robusto','coracao_vital'], desc:'Combinação de Manto Robusto + Coração Vital. +20% de HP máximo, e regenera 2% da vida máxima a cada 3 segundos enquanto estiver vivo — ótimo pra batalhas longas, diferente da defesa de impacto da Muralha Viva.', effect:{hpPct:0.20, appliesTo:'all'}},
  nucleo_rachadura:{name:'Núcleo da Rachadura', kind:'relíquia · chefe', isRelic:true, desc:'Relíquia rara — só dropa de chefe. +35% de dano e +20% de velocidade, em qualquer Stack User. Não tem receita, não dá pra comprar.', effect:{atkPct:0.35, speedPct:0.20, appliesTo:'all'}},
  fragmento_corrompido:{name:'Fragmento Corrompido', kind:'relíquia · chefe', isRelic:true, desc:'Relíquia rara — só dropa de chefe. A cada abate, 25% de chance de curar 15% da vida máxima na hora. Não tem receita, não dá pra comprar.', effect:{onKillHealChance:0.25, onKillHealPct:0.15, appliesTo:'all'}},
  coroa_ferro:{name:'Coroa de Ferro', kind:'relíquia · chefe', isRelic:true, desc:'Relíquia rara — só dropa de chefe. +220 de HP máximo e 18% de redução de dano, em qualquer Stack User. Não tem receita, não dá pra comprar.', effect:{hpFlat:220, dmgReductionPct:0.18, appliesTo:'all'}},
};
...
const ROLE_STAR_PROFILE = {
  ...
  'Pilar da Perseverança': {hp:0.20, atk:0.10, speed:0.03, defFlat:0.04},
...
// (a seed do personagem inicial acontece no fluxo de verdade — openStarterPick/hardResetProgress
// pro PvE, startPvpSetup pro PvP, ou resumeSavedGame — não aqui, senão a Ferrha aparecia em
// "Meu Time" antes até da pessoa começar a primeira partida.)
...
window.addEventListener('beforeunload', (e)=>{
  if(!inMatch) return;
  e.preventDefault();
  e.returnValue = ''; // exigido pelos navegadores pra mostrar o aviso nativo de confirmação
});
...
// Recordes pessoais — separados do save normal do jogo (que reseta ao recarregar a página).
// Ficam guardados de verdade, pro Perfil mostrar o histórico da pessoa.
...
function updateCoinBadge(){
  document.getElementById('coin-count').textContent = testMode ? '∞' : coins;
...
const ACHIEVEMENTS = {
  ...
  wave40: {icon:'🌌', name:'Além do Limite', desc:'Alcance a onda 40'},
  wave50: {icon:'🕳️', name:'No Coração da Rachadura', desc:'Alcance a onda 50'},
  ...
  double_boss_slayer: {icon:'⚠️', name:'Chefe em Dobro', desc:'Derrote um chefe que veio com poder emprestado E passiva rara (onda 25+)'},
  ...
  no_deaths_20: {icon:'🕊️', name:'Intocável', desc:'Chegue na onda 20 sem perder nenhum Stack User na partida'},
  ...
  team_expansion_first: {icon:'📈', name:'Mais Espaço', desc:'Compre a primeira expansão de time (onda 40+)'},
  team_expansion_full: {icon:'🏰', name:'Exército Completo', desc:'Leve o time até 8 Stack Users'},
  voltra_wipe: {icon:'⚡', name:'Explosão Suprema', desc:'Vença uma onda com a explosão da Voltra eliminando o último inimigo'},
  nerith_swarm: {icon:'🐙', name:'Enxame Completo', desc:'Tenha os 4 tentáculos da Nerith vivos ao mesmo tempo'},
  mono_element: {icon:'🔮', name:'Pureza Elemental', desc:'Vença uma onda com um time formado só por um elemento'},
  rich: {icon:'💰', name:'Bolso Cheio', desc:'Tenha 1000 moedas guardadas ao mesmo tempo'},
  segredo_onda60: {icon:'👁️', name:'Olá? AI MEU DEUS', desc:'', secret:true},
};
...
// Conquistas da partida ATUAL (reinicia a cada nova partida) — é isso que vai pro Placar,
// não a coleção vitalícia inteira. O Perfil continua usando unlockedAchievements (vitalícia).
...
function buildMyTeamSnapshot(){
  return Object.entries(owned).map(([champId, prog])=>{
    ...
    if(prog.relicId && ITEM_CATALOG[prog.relicId]) itemNames.push(ITEM_CATALOG[prog.relicId].name+' (relíquia)');
...
async function postMyResult(){
  ...
  if(!window.__lb || !window.__lb.ready){ statusEl.textContent = 'Placar indisponível no momento (sem conexão com o banco de dados).'; return; }
  const team = buildMyTeamSnapshot();
  if(team.length===0){ statusEl.textContent = 'Você ainda não tem nenhum Stack User pra postar — joga uma partida primeiro!'; return; }
  let playerName = 'Anônimo';
  try{ playerName = localStorage.getItem(PROFILE_NAME_KEY) || 'Anônimo'; }catch(e){}
  if(!playerName.trim()) playerName = 'Anônimo';
  ...
  try{
    ...
    statusEl.textContent = 'Postado! Já aparece na lista abaixo.';
    renderLeaderboard();
  }catch(e){
    statusEl.textContent = 'Não deu pra postar agora — tenta de novo daqui a pouco.';
    console.error(e);
  }
}
async function likeEntry(entryId, btnEl){
  ...
  const liked = getLikedEntries();
  if(liked.has(entryId)) return; // já curtiu antes nesse navegador
  btnEl.disabled = true;
  try{
...
async function renderLeaderboard(){
  ...
  if(!window.__lb || !window.__lb.ready){
    statusEl.textContent = 'Placar indisponível no momento (sem conexão com o banco de dados).';
    listEl.innerHTML = '';
    return;
  }
  ...
  try{
    ...
    statusEl.textContent = snap.empty ? 'Ninguém postou ainda — seja o primeiro!' : '';
    const liked = getLikedEntries();
    listEl.innerHTML = snap.docs.map(d=>{
      ...
      const teamHtml = (data.team||[]).map(u=>`
        <div style="padding:6px 8px; background:rgba(0,0,0,0.2); border:1px solid #3a3f47; border-radius:4px; font-size:11px;">
          ...
          <strong>${u.name}</strong> — ${starIcons(u.stars)} Nv.${u.level}
          ${u.items.length ? `<div style="color:var(--gold); margin-top:2px;">🎒 ${u.items.join(', ')}</div>` : '<div style="color:var(--steel); margin-top:2px;">Sem itens</div>'}
        </div>
      `).join('');
      return `
        <div style="padding:14px; background:rgba(0,0,0,0.15); border:1px solid #3a3f47; border-radius:6px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
            <div style="font-family:'Oswald',sans-serif; font-size:15px; color:#eae4d8;">${data.playerName||'Anônimo'}</div>
            <div style="color:var(--gold); font-family:'JetBrains Mono',monospace; font-size:13px;">🌊 Onda ${data.maxWave||0}</div>
          </div>
          ...
          <button class="ghost-btn like-btn${alreadyLiked?' liked':''}" data-entry-id="${id}" ${alreadyLiked?'disabled':''}>
            ${alreadyLiked?'❤️':'🤍'} <span class="like-count">${data.likes||0}</span>
          </button>
        </div>
      `;
    }).join('');
    ...
    statusEl.textContent = 'Não deu pra carregar o placar agora — tenta de novo daqui a pouco.';
    console.error(e);
  }
}
...
function renderProfile(){
  ...
  gridEl.innerHTML = ids.map(id=>{
    ...
    const icon = (def.secret && !unlocked) ? '❓' : def.icon;
  ...
  document.getElementById('profile-max-wave').textContent = personalRecords.maxWave>0 ? `🌊 Maior onda alcançada: ${personalRecords.maxWave}` : '🌊 Ainda não alcançou nenhuma onda.';
  ...
  if(dmgEntries.length===0){
    dmgListEl.innerHTML = '<div class="subtitle">Nenhum dano registrado ainda — entra numa partida!</div>';
  } else {
    ...
    if(def.secret && !unlocked){
      return `
        <div style="display:flex; align-items:center; gap:12px; padding:10px 12px; background:rgba(0,0,0,0.15); border:1px solid #3a3f47; border-radius:6px;">
          ...
          <div>
            <div style="font-family:'Oswald',sans-serif; font-size:14px; color:#8b95a3;">??? 🔒</div>
...
function unlockAchievement(id){
  if(mode==='pve' && wave < 7) return; // conquistas só liberam a partir da onda 7 — muito fácil pegar 2 de uma vez na primeira rodada
  matchAchievements.add(id); // conta pra essa partida mesmo que já fosse vitalícia
...
// Aviso de atualização — só dispara pra quem JÁ tinha jogado numa versão anterior
// (não incomoda quem tá abrindo o jogo pela primeira vez). Não é obrigatório recarregar.
...
try{
  ...
  if(seenVersion && seenVersion !== GAME_VERSION){
    setTimeout(()=>{
      showAppToast('🔄 Tem uma atualização nova disponível! Quando terminar sua partida, recarregue a página pra receber as novidades (não é obrigatório).', 10000);
    }, 2500);
  }
  localStorage.setItem('ferroLancaSeenVersion', GAME_VERSION);
}catch(e){}

// Bolinha de "não lido" nas Notas — diferente do aviso acima, essa mostra até pra quem
// tá jogando pela primeira vez (o importante aqui é "você já leu ISSO", não "mudou algo").
try{
...
document.getElementById('mobile-mode-ask-yes').addEventListener('click', ()=>{
  ...
  showAppToast('📱 O modo de celular foi ativado — você pode trocar isso nas Configurações.');
});
...
(function(){
  ...
  if(sendBtn) sendBtn.addEventListener('click', ()=>{
    const subject = (subjectEl.value || 'Sugestão para Ferro & Lança').trim();
    const body = (bodyEl.value || '').trim();
    if(!body){ bodyEl.focus(); return; }
    const target = atob(FB_TARGET_B64);
    const mailUrl = `mailto:${target}?subject=${encodeURIComponent('[Ferro & Lança] '+subject)}&body=${encodeURIComponent(body)}`;
...
// Quando um Stack User entra num time que já tá em andamento, ele pula pra perto do nível
// do resto do grupo (em vez de sempre nascer no 1) e ganha XP turbinado até alcançar de vez.
function grantCatchUpJoin(champId){
  ...
  if(startLevel > prog.level){
    ...
    prog.xp = 0;
    log(`${CHAMPION_CATALOG[champId].name} entra no time já no nível ${startLevel}, pra não ficar tão atrás do resto do grupo!`, 'hl');
  }
  if(minVeteranLevel - prog.level > 0) newcomerChamps.add(champId);
}

/* ============ BÊNÇÃOS ============ */
// A cada 2 chefes derrotados (chefe duplo conta como 1), o jogador escolhe 1 de 3 bênçãos —
// cada uma tem um lado bom e um lado ruim, sem exceção.
const BLESSINGS = {
  furia_coletiva: {name:'Fúria Coletiva', icon:'⚔️', pro:'+15% de dano em todo o time', con:'-10% de vida máxima em todo o time'},
  passo_leve: {name:'Passo Leve', icon:'💨', pro:'+20% de velocidade de movimento e ataque em todo o time', con:'-15% de redução de dano em todo o time'},
  forja_barata: {name:'Bênção do Ferreiro', icon:'🔨', pro:'Itens combináveis custam 30% menos moedas pra montar', con:'+10% de dano recebido em todo o time'},
  vinculo_duplo: {name:'Vínculo Duplo', icon:'🔗', pro:'2 Stack Users aleatórios passam a poder equipar até 5 itens', con:'Esses 2 mesmos Stack Users ganham 25% menos XP'},
  reforco_tardio: {name:'Reforço Tardio', icon:'📯', pro:'Todo Stack User comprado a partir de agora entra 8 níveis mais alto', con:'Personagens custam 15% mais moedas na loja'},
  folego_extra: {name:'Fôlego Extra', icon:'❤️', pro:'+18% de vida máxima em todo o time', con:'-10% de velocidade de ataque em todo o time'},
};
let activeBlessings = []; // ids das bênçãos escolhidas nessa partida
let bossDefeatCount = 0;
let bossCountedThisWave = false;
let pendingBlessingChoice = false;
let extraSlotChampIds = []; // preenchido pela bênção Vínculo Duplo
...
// Limitador de economia: quando o time já tá muito completo (estrelas máximas / itens cheios),
// o ganho de moeda cai — força escolher entre comprar item OU a expansão de time lá pela onda 40.
function computeCompletionPenalty(){
  ...
  return 1 - completion*0.45; // até 45% de redução quando o time tá 100% maxado
}
...
function showBlessingChoice(onResolved){
  ...
  if(pool.length===0){
    ...
    el.innerHTML = `<div class="champ-card" style="border-color:var(--ember);">
      <div class="champ-name" style="color:var(--ember);">🏛️ Notificação da Rachadura</div>
      <div class="champ-desc">Você não tem mais bênçãos novas pra receber — mas ainda precisa pagar imposto sobre as anteriores. Sem direito a recurso.</div>
      <div class="champ-stats" style="color:var(--ember);">Cobrado: ${paid} moedas${paid<taxCost?' (tudo que você tinha)':''}.</div>
    ...
    log(`🏛️ Imposto sobre bênçãos cobrado: ${paid} moedas.`, 'hl');
  ...
  el.innerHTML = shuffled.map(id=>{
    ...
    return `<div class="champ-card" style="cursor:pointer;" data-blessing="${id}">
      ...
      <div class="champ-name" style="color:var(--steel);">🚫 Não quero bênção</div>
      <div class="champ-stats">Paga ${skipCost} moedas pra dispensar todas as três — sem pró, sem contra.</div>
    </div>`;
  ...
  if(skipBtn){
    skipBtn.addEventListener('click', ()=>{
      ...
      coins -= skipCost;
      updateCoinBadge();
      log(`🚫 Bênção dispensada por ${skipCost} moedas.`, 'hl');
      document.getElementById('blessing-overlay').classList.remove('show');
      pendingBlessingChoice = false;
      if(onResolved) onResolved();
    });
...
function applyBlessing(id){
  ...
  const b = BLESSINGS[id];
  log(`✨ Bênção escolhida: ${b.name} — ${b.pro} / ${b.con}`, 'hl');
  if(id==='vinculo_duplo'){
    ...
    log(`🔗 ${extraSlotChampIds.map(id=>CHAMPION_CATALOG[id].name).join(' e ')} agora podem equipar até 5 itens!`, 'hl');
  }
}

const MAX_OWNED_CHAMPS_BASE = 5;
const TEAM_EXPANSION_COSTS = [1500, 2200, 3000]; // custo de cada tier: 5→6, 6→7, 7→8
const TEAM_EXPANSION_UNLOCK_WAVE = 40;
let teamExpansionTier = 0; // 0 a 3 — cada tier soma +1 no limite de personagens
...
function renderShop(){
  ...
  if(mode==='pve' && wave>=TEAM_EXPANSION_UNLOCK_WAVE && teamExpansionTier<TEAM_EXPANSION_COSTS.length){
    ...
    card.innerHTML = `
      <div class="champ-name" style="color:var(--gold);">⭐ Expandir o Time</div>
      <div class="champ-desc">Libera espaço pra mais 1 Stack User no time — limite atual: ${getMaxOwnedChamps()}, vira ${nextCap}.</div>
      <div class="champ-stats" style="color:var(--gold);">${expCost} moedas</div>
      <button class="main-btn" style="margin-top:8px;width:100%;" ${coins<expCost?'disabled':''}>Comprar expansão</button>
    `;
    card.querySelector('button').addEventListener('click', ()=>{
      ...
      teamExpansionTier++;
      sfxCoin();
      log(`Time expandido! Agora dá pra ter até ${getMaxOwnedChamps()} Stack Users.`, 'hl');
  ...
  sortedChampEntries.forEach(([id,def])=>{
    ...
    if(isOwned){
      ...
      ownedBlock = `
        <div class="champ-stats" style="color:var(--ember);">Time no limite de ${getMaxOwnedChamps()} — troque alguém por ${def.name}</div>
...
// pra não precisar editar cada item existente — é só uma camada de "curadoria").
const ITEM_CLASS_TAGS = {
  ...
  // relíquias de chefe não entram em nenhum filtro de classe — só aparecem em "Todos"
};
...
function renderItems(){
  ...
  sortedItemEntries.forEach(([id,def])=>{
    ...
    let actionBlock;
    if(def.isRelic){
      actionBlock = `<div class="champ-stats" style="color:var(--gold);">✨ Só dropa de chefe — não dá pra comprar</div>`;
    } else if(def.recipe){
      ...
      actionBlock = inMatch
        ? `<button class="main-btn buy-btn" data-buyitem="${id}" ${coins<itemCost?'disabled':''}>Comprar — ${itemCost} 🪙</button>`
        : `<div class="champ-stats" style="color:var(--steel);">Só dá pra comprar durante uma partida</div>`;
    }
    card.innerHTML = `
...
const RECOMMENDED_ITEMS = {
  ...
  nyx: {items:['furia_crescente','botas_lutador'], note:'Crítico e velocidade de ataque — ela já esquiva bastante sozinha, então é ir no ataque rápido.'},
  kael: {items:['amplificador','nucleo_eterno','presas_sangrentas'], note:'Amplifique o frenesi: mais duração, mais vampirismo, mais dano — as Presas Sangrentas dão ainda mais sustentação corpo a corpo.'},
  ...
  ima: {items:['amplificador','placa'], note:'Fortalece a aura de defesa pros aliados de metal e aguenta mais no corpo a corpo — ela é 100% corpo a corpo agora.'},
  frosk: {items:['punho_serra','coracao_vital'], note:'Perfuração de armadura pra furar tanque inimigo, e mais vida pra aguentar o combate corpo a corpo.'},
  gelida: {items:['amplificador','nucleo_eterno'], note:'Aumenta a duração do espectro e a cura ao renascer.'},
  raio: {items:['botas_lutador','carniceiro'], note:'Velocidade de ataque pra ativar o combo mais rápido, e o Carniceiro dá perfuração + vampirismo pra sustentar a briga.'},
  ...
  nerith: {items:['coracao_vital','manto_robusto'], note:'Ela mesma é frágil e não briga direto — o importante é sobreviver enquanto os tentáculos fazem o trabalho.'},
  voltra: {items:['mira','olho_falcao'], note:'A onda de choque dela escala com velocidade de ataque — quanto mais rápida, mais forte a explosão.'},
  jedegar: {items:['coracao_vital','manto_robusto','placa'], note:'Ela vive de sobreviver — vida e defesa extra deixam os pilares e a passiva de morte mais fáceis de aproveitar.'},
  shava: {items:['presas_sangrentas','botas_lutador'], note:'Vampirismo pra sustentar o combo de golpes, e velocidade pra chegar nos 3/6 golpes mais rápido.'},
};
...
function renderRoster(){
  ...
  Object.entries(owned).forEach(([id,prog])=>{
    ...
    for(let slot=0; slot<slotCount; slot++){
      ...
      const title = current ? ITEM_CATALOG[current].name : 'Slot vazio — arraste um item aqui';
    ...
    const relicIcon = prog.relicId ? itemIconSVG(prog.relicId, 18) : '▢';
    const relicSlotHtml = `
    ...
    card.innerHTML = `
      ...
      <div class="champ-stats" style="margin-top:6px;">Itens (até 3): <strong style="color:var(--gold);">${equippedNames.length?equippedNames.join(', '):'Nenhum'}</strong>${prog.relicId?` · Relíquia: <strong style="color:var(--gold);">${ITEM_CATALOG[prog.relicId].name}</strong>`:''}</div>
      ${slotsHtml}
    `;
    el.appendChild(card);
  });
  el.querySelectorAll('[data-equip-slot]').forEach(slotEl=>{
    slotEl.addEventListener('click', ()=>{
      ...
      if(!oldItemId) return; // slot vazio — clique não faz nada, é só pra arrastar
...
function equipItemFromDrag(itemId, champId, targetSlotEl){
  ...
  if(targetSlotEl.dataset.equipRelic){
    if(!isRelic) return false; // só relíquia entra no slot de relíquia
  ...
  if(targetSlotEl.dataset.equipSlot){
    if(isRelic) return false; // relíquia não entra em slot normal
    if((itemInventory[itemId]||0)<=0) return false;
...
// gera o HTML dos slots de equipamento (itens + relíquia) de um personagem —
...
function buildEquipSlotsHtml(champId){
  ...
  const relicIcon = prog.relicId ? itemIconSVG(prog.relicId, 16) : '▢';
  html += `<div class="equip-slot${prog.relicId?' filled':''}" data-equip-relic="${champId}" title="${prog.relicId?ITEM_CATALOG[prog.relicId].name:'Slot de relíquia'}">${relicIcon}</div>`;
  html += '</div>';
  return html;
}
...
function renderInventoryPanel(gridId){
  ...
  if(entries.length===0){
    grid.innerHTML = '<div class="inv-empty-msg">Nenhum item livre no momento — compre na Loja.</div>';
    return;
  }
  entries.forEach(([iid,count])=>{
    ...
    tile.title = ITEM_CATALOG[iid].name + ' — clique pra vender, ou arraste pra equipar';
...
/* ============ ARRASTAR ITEM (ponteiro — funciona no mouse e no toque) ============ */
/* Distingue clique (abre venda) de arraste (equipar ou soltar na lixeira pra vender)
   por um limiar de movimento — só vira "arraste de verdade" depois de mexer uns pixels. */
let dragState = null;
const DRAG_RENDER_TARGETS = { roster: renderRoster, position: renderPositionScreen };
const RELIC_SELL_XP = 30; // XP total dado ao vender uma relíquia de chefe, dividido entre quem for escolhido

let mobileEquipMode = false;
let selectedInventoryItem = null; // { itemId, renderTargetKey } — usado só no modo celular
function onInvTileClickSelect(itemId, tileEl, renderTargetKey){
  if(selectedInventoryItem && selectedInventoryItem.itemId===itemId && selectedInventoryItem.tileEl===tileEl){
    // tocou de novo no mesmo item já selecionado — cancela a seleção e abre a venda
    selectedInventoryItem = null;
    document.querySelectorAll('.inv-tile.selected-for-equip').forEach(t=>t.classList.remove('selected-for-equip'));
    openSellFlow(itemId);
    return;
...
function onInvTilePointerDown(e, itemId, renderTargetKey){
  ...
  function upCheck(){
    window.removeEventListener('pointermove', moveCheck);
    window.removeEventListener('pointerup', upCheck);
    if(!started) openSellFlow(itemId); // foi só um toque/clique — abre a opção de vender
  }
...
function onItemDragEnd(e){
  ...
  // checa o que está embaixo do ponteiro ANTES de esconder a lixeira/fantasma,
  // senão o hit-test não encontra mais nada ali (era a causa do item "voltar sozinho")
  const under = document.elementFromPoint(e.clientX, e.clientY);
  const trashEl = under ? under.closest('#item-trash-zone') : null;
  const slotEl = (!trashEl && under) ? under.closest('.equip-slot') : null;
...
function openNormalSellConfirm(itemId){
  ...
  document.getElementById('sell-confirm-detail').textContent = `Vender por ${sellValue} 🪙 (20% do valor original)`;
...
// Vida escala separado do ataque, e mais forte — os inimigos aceleram (quase quadrático) depois
// da onda 10, então a vida dos Stack Users também precisa acelerar, senão eles ficam pra trás.
...
function awardXp(champId, amount){
  ...
  if(extraSlotChampIds.includes(champId)) amount = amount*0.75; // contra do Vínculo Duplo
  const prog = owned[champId];
  prog.xp += amount;
...
function catchUpMultiplier(champId){
  ...
  return Math.min(10, 1 + gap*0.7); // quanto maior o atraso, mais forte o bônus, até 10x
}
...
let currentBiome = 'grama'; // 'grama' | 'rachadura' — tema visual do chão, escala com a onda/rodada
const BIOME_LABELS = { grama:'Campo verde', rachadura:'Terreno corrompido' };
function hexNoise(q,r,salt){
  // hash determinístico simples — o mesmo hex sempre dá o mesmo valor pro mesmo salt, sem precisar guardar estado
  ...
  allHexes.forEach(h=>{
    if(axisValue(h)===0){
      ...
      if(jedegarStructures[hk]){
        ...
        spawnFloatText(pp.x, pp.y-30, 'PILAR DESTRUÍDO!', '#e8e0ff');
...
// Progresso da Jedegar que precisa sobreviver entre rodadas (as unidades são recriadas
// do zero a cada rodada, mas "5 pilares no total" e o gatilho de 30% de vida são
// marcos de PARTIDA inteira, não de uma rodada só).
...
function renderPositionScreen(){
  ...
  posTeamIds.forEach(cid=>{
    ...
    chip.addEventListener('click', (e)=>{
      if(e.target.closest('.equip-slot')) return; // clique num slot não seleciona pra posicionar
...
function onUnitPointerDown(e, cid){
  ...
  function upCheck(){
    ...
    window.removeEventListener('pointerup', upCheck);
    if(!started){
      // foi só um clique — mantém o comportamento original (seleciona, clica no hex depois)
      posSelectedChamp = (posSelectedChamp===cid ? null : cid);
      renderPositionScreen();
    }
  }
...
// Passivas secretas entre duplas específicas — como as unidades são recriadas do zero a cada
// rodada/formação de time, isso já "desativa sozinho" se o jogador tirar alguém do time (a
// próxima vez que buildTeams rodar, a dupla simplesmente não vai mais estar junta).
function applySecretPassives(sideUnits){
  ...
  if(jedegar && terrus){
    ...
    log('✨ Jedegar e Terrus têm uma passiva secreta!', 'hl');
  }
  ...
  if(shava && kael){
    ...
    log('✨ Shava e Kael têm uma passiva secreta!', 'hl');
  }
}
...
function randomEnemyWave(waveNum){
  const keys = Object.keys(ENEMY_CATALOG).filter(k=>k!=='eco_rachadura'); // exclusivo da onda 60, nunca sorteado à toa
  ...
  if(isBossWave){
    ...
    for(let bi=0; bi<bossCount; bi++){
      ...
      let bossMult;
      if(waveNum===5) bossMult = {hp:0.7, atk:0.4, stars:1}; // primeiro chefe: bem fraco de propósito, dá pra passar com qualquer time
      ...
      const expansionMultBoss = 1 + teamExpansionTier*0.12;
      // quando vêm dois juntos, cada um fica um pouco mais fraco individualmente — senão dobra a dificuldade de vez
      ...
      // discípulos: o chefe pode nascer com 2-3 inimigos fracos protegendo ele, senão o time foca e mata rápido demais
  ...
  return picks.map((k,i)=>{
    ...
    if(teamExpansionTier>0){
      // time expandido enfrenta inimigos proporcionalmente mais fortes, não só mais numerosos
...
function startPveRun(){
  ...
  document.getElementById('roundinfo').textContent = `Onda 1 — ${BIOME_LABELS[currentBiome]} — ${WEATHER_LABELS[currentWeather]}`;
  resetRoundTracking();
...
function logReinforcements(wavePicks, waveNum){
  if(lastWaveEvent==='boss'){
    ...
    if(bosses.length>=2){
      log(`⚠️⚠️ DOIS CHEFES na mesma onda! ${bosses.map(w=>ENEMY_CATALOG[w.key].name).join(' e ')} apareceram bem mais fortes.`, 'hl');
    } else {
      log(`⚠ Onda de chefe! ${ENEMY_CATALOG[bosses[0].key].name} apareceu bem mais forte${bosses[0].scaled.stars>=4?' — E COM 4 ESTRELAS!':''}.`, 'hl');
    }
    if(disciples.length>0){
      log(`🗡️ O chefe não veio sozinho — ${disciples.length} inimigos vieram protegendo!`, 'hl');
    }
    bosses.forEach(w=>{
      if(w.borrowedPower){
        log(`🌀 ${ENEMY_CATALOG[w.key].name} veio com uma habilidade emprestada: ${BORROWED_POWER_NAMES[w.borrowedPower] || w.borrowedPower}!`, 'hl');
        if(w.borrowedChampPower){
          log(`⚠ E também com uma passiva rara: ${BORROWED_CHAMP_POWER_NAMES[w.borrowedChampPower]}! Chefe em dobro.`, 'hl');
        }
      }
    });
...
function spawnCorruptedCreatures(waveNum){
  ...
  for(let i=0;i<count;i++){
    ...
    // hordas: morrem rápido (menos vida), mas causam bastante dano — e explodem ao morrer
...
function startPvpRound(){
  ...
  document.getElementById('roundinfo').textContent = `Rodada ${pvpRound} — Jogador 1 (esq.) vs Jogador 2 (dir.) — ${BIOME_LABELS[currentBiome]} — ${WEATHER_LABELS[currentWeather]}`;
...
function resumeSavedGame(){
  ...
  scheduleNextFracture(performance.now());
  document.getElementById('roundinfo').textContent = `Onda ${wave} — ${BIOME_LABELS[currentBiome]} — ${WEATHER_LABELS[currentWeather]}`;
  const defaultSlots = getPlayerZoneSlots();
...
function quitToMenu(){
  ...
  detailEl.textContent = `Onda ${wave} — maior onda geral: ${Math.max(wave, bestWaveEver)}. Seu time e itens aparecem no Placar pra todo mundo ver.`;
  overlay.classList.add('show');
...
function nearestEnemy(u){
  ...
  if(!taunters.length && u.range>1 && hasItem(u,'olho_falcao')){
    // Olho de Falcão: em vez de mirar no mais próximo, foca sempre no inimigo com menos vida ao alcance
...
function checkPortalTeleport(u){
  ...
  for(const [a,b] of allPairs){
    ...
    if(dest){
      // evita ficar entrando e saindo do mesmo portal toda hora — mas depois de 5s libera de novo,
      // pra funcionar como saída de emergência se a unidade ficar presa num canto.
...
// marco de 5 pilares), e o golpe que atinge um pilar libera +15% temporário por 3s.
// Não empilha entre pilares diferentes — pega sempre o melhor bônus disponível.
...
function jedegarEnemyDefenseDebuff(u){
  ...
  let found = false;
  Object.values(jedegarStructures).forEach(s=>{
    if(s.ownerTeam === u.team) return; // só afeta quem NÃO é do time da Jedegar
    if(hexDistance(s, u) > 2) return;
    const owner = units.find(o=>o.id===s.ownerId);
...
function triggerEmberItem(u){
  ...
  targets.forEach(t=>{
    applyDamage(fireSource, t, emberBaseDmg(u), '🔥', true);
    ...
    if(u.element==='eletrico' && t.alive){
      ...
      if(Math.random() < 0.4){
        ...
        if(chainCandidates.length){
          ...
          applyDamage(shockSource, chainTarget, emberBaseDmg(u), '⚡', true);
  ...
  emberEvents.forEach(ev=>{
    ...
    if(t && t.alive){
      if(ev.type==='tick'){
        ...
        applyDamage({element:'fogo', team: t.team==='player'?'enemy':'player', champId:'brasa', name:'Brasa Selvagem'}, t, emberBaseDmg(src), '🔥', true);
...
// Falas soltas individuais — cada personagem tem a própria, sem depender de estar em dupla com ninguém.
const SOLO_LINES = {
  ferrha: {levelup:'Fico mais firme a cada dia.', battlestart:'Ninguém passa enquanto eu estiver de pé.'},
  voss: {levelup:'Minha mira melhora. As perguntas continuam as mesmas.', battlestart:'Vamos ver o que esse lugar esconde.'},
  nyx: {levelup:'Nem eu percebi.', battlestart:'Vocês nem vão me ver chegar.'},
  kael: {levelup:'Mais fogo. Mais poder. Nunca é suficiente.', battlestart:'Vamos ver até onde eu aguento dessa vez.'},
  terrus: {levelup:'A vila que eu perdi teria orgulho disso.', battlestart:'Fico de guarda. Como sempre.'},
  pyra: {levelup:'Cada nível é mais um passo perto de entender a Rachadura.', battlestart:'Deixa eu estudar isso de perto.'},
  glacia: {levelup:'Espero que isso seja o suficiente pra mantê-los vivos.', battlestart:'Fico de olho em todo mundo.'},
  zeph: {levelup:'Mais rápida. Sempre mais rápida.', battlestart:'Última a chegar, primeira a sair.'},
  ima: {levelup:'Ainda não sei o que significa "querer". Mas isso ajuda.', battlestart:'Tudo que reluz vem pra perto de mim.'},
  frosk: {levelup:'Mais forte. Bom.', battlestart:'Chega perto que eu congelo.'},
  gelida: {levelup:'Mais um nível. Mais um empréstimo contra a morte.', battlestart:'Dessa vez eu não vou ficar pra trás.'},
  raio: {levelup:'Mais carga. Mais risco. Perfeito.', battlestart:'Vamos ver se eu aguento até o fim dessa vez.'},
  shecry: {levelup:'Quanto mais eu aguento, mais forte eu fico.', battlestart:'Tentem me machucar. Vejam o que acontece.'},
  nerith: {levelup:'ELE está satisfeito com esse progresso.', battlestart:'Eles já sabem que estamos aqui.'},
...
function trySoloLine(champId, moment){
  if(interactionFiredThisRound) return; // já teve uma interação nessa rodada — espera a próxima
  const lines = SOLO_LINES[champId];
  if(!lines || !lines[moment]) return;
...
const DUO_BANTER = [
  ...
  {pair:['jedegar','terrus'], lines:[
    {speaker:'jedegar', text:'Terrus... é você mesmo? O guardião que protegeu minha vila?'},
    {speaker:'terrus', text:'...não me lembro de você.'},
    {speaker:'jedegar', text:'Não precisa lembrar. Eu nunca esqueci.'},
  ]},
  {pair:['shava','kael'], lines:[
    {speaker:'shava', text:'Sei de onde você vem. Minha vila nunca esqueceu o que a sua fez.'},
    {speaker:'kael', text:'Eu nem tinha nascido. Mas se quer briga, eu aceito.'},
    {speaker:'shava', text:'Não é briga. É questão de honra.'},
  ]},
...
let interactionFiredThisRound = false; // garante só 1 interação (dupla, trio ou solo) por rodada
...
function applyDamage(attacker, target, baseDmg, tag, ignoreDefense){
  ...
  if(countItem(attacker,'furia_crescente')>0 && Math.random()<0.18){
    ...
    spawnFloatText(attacker.rx, attacker.ry-36, 'CRÍTICO!', '#e8c250');
  }
  if(attacker.champId==='gelida' && attacker.gelidaDmgBuffUntil && now < attacker.gelidaDmgBuffUntil) dmg = Math.round(dmg*2); // fúria glacial pós-renascimento
  ...
  if(target.champId==='jedegar' && target.hp>0 && target.hp/target.maxhp<=0.30){
    ...
    if(!jprog.lowHpUsed){
      ...
      if(nearEnemy){
        ...
        knockbackUnit(nearEnemy, target, 4); // ela foge sobre uma fileira de pedra recém-criada
      }
      spawnCastEffect(target.rx, target.ry, '#8a6a4a');
      spawnFloatText(target.rx, target.ry-40, 'PILAR DE FUGA!', '#8a6a4a');
      log(`${target.name} ergue um pilar sob o inimigo mais próximo, arremessando-o longe, e foge sobre uma fileira de pedra recém-criada!`, 'hl');
  ...
  if(dmg>0 && attacker.champId==='shava'){
    ...
    if(attacker.shavaComboTargetHits % 6 === 0){
      spawnFloatText(attacker.rx, attacker.ry-58, 'Ápice da tormenta!', '#c9d9e8', 'dramatic');
      log(`${attacker.name}: "Ápice da tormenta!"`, 'hl');
      ...
      if(vortexNearby.length){
        ...
        log(`${attacker.name} cria um vórtice, empurrando e atordoando ${vortexNearby.length} inimigo${vortexNearby.length>1?'s':''} ao redor!`, 'hl');
      }
    }
  }
  if(target.hp<=0){
    if((target.champId==='kael' || target.borrowedChampPower==='kael') && !target.frenzyUsed){
      ...
      if(target.team==='corrupted'){
        ...
        if(nearby.length){
          ...
          nearby.forEach(o=> applyDamage(explodeSource, o, explodeDmg, '💥', true));
      ...
      if(target.champId==='jedegar'){
        const jedegarDeathPhrase = 'A morte é apenas uma nova etapa de nossa ascensão!';
        spawnCastEffect(target.rx, target.ry, '#8a6a4a');
        ...
        if(jedegarAllies.length){
          ...
          spawnFloatText(jedegarChosen.rx, jedegarChosen.ry-34, 'BÊNÇÃO DE JEDEGAR', '#8a6a4a');
          log(`${target.name} ergue um círculo protetor ao redor de ${jedegarChosen.name} — escudo, dano e velocidade de ataque por 5 segundos!`, 'hl');
        }
      ...
      if(target.isTentacle){
        ...
        if(parent && parent.alive){
          ...
          if(remaining===3) parent.tentacleCooldown = 1500; // ela tinha o limite de 4, abriu vaga — reabastece rápido
        }
      }
      if(mode==='pve' && attacker.team==='player' && (target.team==='enemy' || target.team==='corrupted')){
        ...
        waveXpPool += xpAmt; // vai pro fundo comum — dividido igualmente com o time no fim da onda
        ...
        if(hasItem(attacker,'fragmento_corrompido') && Math.random()<0.25 && attacker.alive && attacker.hp<attacker.maxhp){
          ...
          attacker.hp = Math.min(attacker.maxhp, attacker.hp+healAmt);
          spawnFloatText(attacker.rx, attacker.ry-30, `+${healAmt} ✨`, '#9b4fd9');
        }
        if(target.team==='corrupted'){
        ...
        if(target.isBoss){
          ...
          if(Math.random() < 0.4){
            ...
            spawnFloatText(target.rx, target.ry-50, `✨ ${ITEM_CATALOG[relicId].name}!`, '#e8c250', 'dramatic');
            log(`✨ ${target.name} deixou cair uma relíquia: ${ITEM_CATALOG[relicId].name}!`, 'hl');
          }
...
function tryAttackNearbyStructure(u){
  ...
  // libera energia: fortalece os pilares aliados próximos por 3s
  ...
  if(s.hitsTaken >= needed){
    ...
    log('Um pilar de Jedegar foi destruído.', 'sys');
  }
  return true;
}
function doAction(u){
  // pilares da Jedegar viram alvo de verdade — se tiver um por perto, tem uma chance boa
...
function updateBattleLogic(dt){
  if(finisherActive) return; // pausa a física normal enquanto a cinemática do golpe final roda
  const now = performance.now();
  units.forEach(u=>{
    if(!u.alive) return;
      ...
      if(now >= u.ghostUntil){
        ...
        if(target && !target.alive){
          ...
          if(u.champId==='gelida'){
            ...
            u.gelidaDmgBuffUntil = now + 999999; // dura até o fim da onda (unidade é recriada do zero na próxima)
            spawnFloatText(u.rx, u.ry-46, 'FÚRIA GLACIAL!', '#8fd4e8');
            const left = Math.max(0, 2-usedNow);
            log(`${u.name} volta com muito mais força até o fim da onda! (${left} renascimento${left===1?'':'s'} restante${left===1?'':'s'})`, 'hl');
          }
    ...
    if(u.isWave60Boss && u.alive){
      if(now >= u.ecoBlastAt){
        ...
        triggerScreenShake(10, 300);
        setTimeout(()=>{
          targets.forEach(o=>{ if(o.alive) applyDamage(blastSource, o, Math.round(u.atk*1.8), '💥', true); });
        }, 350);
      }
    ...
    if(u.champId==='nerith' && u.alive){
      ...
      if(aliveTentacleCount<4){
        ...
        if(u.tentacleCooldown<=0){
          ...
          u.tentacleCooldown = 2500 + newCount*500; // 1º:2.5s, 2º:3s, 3º:3.5s, 4º:4s
        }
      }
    }
    ...
    if(u.champId==='shava' && u.alive){
      ...
      if(u.shavaKickCooldown<=0){
        ...
        if(inRange.length){
          ...
          // avança até perto do alvo — corrida visual (não teleporte instantâneo), a posição
          // lógica já muda na hora (pro combate ser justo), mas o visual "corre" até lá.
          const occ = occupiedMap();
          ...
          spawnFloatText(landingX, landingY-40, 'Golpe aéreo', '#c9d9e8', 'dramatic');
          log(`${u.name}: "Golpe aéreo"`, 'hl');
          ...
          if(kickTarget.hp>0 && kickTarget.hp/kickTarget.maxhp < 0.10){
            ...
            spawnFloatText(landingX, landingY-58, 'Golpe aéreo: último sopro', '#c9d9e8', 'dramatic');
            log(`${u.name}: "Golpe aéreo: último sopro"`, 'hl');
            ...
            // arremessa o corpo em outro inimigo próximo — atordoa, sem causar dano
            ...
            if(others.length){
              ...
              log(`O corpo de ${kickTarget.name} acerta ${thrown.name} — atordoado, com dano reduzido por 5s.`, 'hl');
            }
            if(mode==='pve' && kickTarget.team==='enemy' && !units.some(o=>o.alive && o.team==='enemy')){
              triggerFinisherSequence(u, ()=>{ checkVictory(); });
            }
          } else {
            // repulsão — dano dobrado, e se bater em algo, atordoa + reduz armadura
            spawnFloatText(landingX, landingY-58, 'Golpe aéreo: onda de ar', '#c9d9e8', 'dramatic');
            log(`${u.name}: "Golpe aéreo: onda de ar"`, 'hl');
            applyDamage(u, kickTarget, u.atk*2, '💨', false);
            if(kickTarget.alive){
              ...
              if(traveled < 5){
                // não percorreu a distância inteira — bateu em obstáculo/parede/estrutura/fenda
                ...
                log(`${kickTarget.name} bate contra um obstáculo — atordoado, com armadura reduzida por 3s.`, 'hl');
                triggerScreenShake(6, 200);
              }
            }
          }
    ...
    if(u.champId==='jedegar' && u.alive){
      ...
      if(u.jedegarPillarTimer<=0){
        ...
        // mira no aliado MAIS DISTANTE dela (não nela mesma) — assim os pilares se espalham pelo
        // time em vez de empilhar tudo perto dela quando ela avança sozinha longe de todo mundo.
        ...
        // nunca constrói no último hex livre ao redor de quem tá recebendo o pilar — sempre
        // precisa sobrar pelo menos 1 saída, senão prende a unidade (ela ou o aliado alvo).
        if(freeNeighbors.length>1){
          ...
          if(!prog.firstPillarAnnounced){
            ...
            spawnFloatText(u.rx, u.ry-40, 'Ascensão do pico celeste', '#c9a53a');
            log(`${u.name}: "Ascensão do pico celeste"`, 'hl');
          }
          if(prog.pillarsBuilt>=5 && !prog.milestoneReached){
            prog.milestoneReached = true;
            spawnFloatText(u.rx, u.ry-56, 'Estamos a um passo do paraíso!', '#e8c250');
            log(`${u.name}: "Estamos a um passo do paraíso!" — os pilares dela ficam permanentemente muito mais fortes!`, 'hl');
            triggerScreenShake(6, 300);
          }
        }
      }
      ...
      if(enemyAlive===false){
        ...
        if(wave>30){
          ...
          if(penalty < 0.98){
            ...
            coinBonusMsg += ` (-${Math.round((1-penalty)*100)}% por time já muito completo)`;
          }
        }
...
function updateGlobalPrepTimer(t){
  ...
  badge.textContent = `⏱ Próxima onda: ${t}s`;
  if(t<=10){
  ...
  units.filter(u=>u.alive).forEach(u=>{
    ...
    if(u.champId==='shava' && u.shavaDashStartAt && now - u.shavaDashStartAt < u.shavaDashDuration){
      ...
      if(!u.shavaLastTrailAt || now - u.shavaLastTrailAt > 35){
        ...
        spawnCastEffect(drawX, drawY, '#c9d9e8'); // rastro de vento atrás dela durante a corrida
      }
    }
    if(u.attackAnim){
    ...
    if(u.champId==='shava' && u.alive){
      ...
      if(chargeProgress > 0.12){
        const ringCount = 1 + Math.floor(chargeProgress*3); // 1 a 4 anéis conforme carrega
        ...
        if(chargeProgress > 0.85){
          // quase pronta pro chute — um brilho extra pra avisar que tá quase ativando
...
// batalha com uma transição suave (fade), pensando num futuro personagem que controla o clima.
...
let finisherActive = false; // pausa a lógica de batalha enquanto a cinemática do golpe final roda

// Só considera "golpe final" quando o abate vem de uma passiva/habilidade especial em andamento —
// não de um ataque básico qualquer.
// Vários efeitos especiais (combo do Raio, corrente da Voltra, etc) aplicam dano usando um objeto
// genérico só pra registrar a origem, em vez da unidade de verdade — isso resolve pra unidade real,
...
// Dispara a cinemática de "golpe final", estilo jogo de luta: vinheta fechando, retrato do
// personagem no centro, câmera dramática nele, flash de impacto, e só então libera o resultado.
function triggerFinisherSequence(killer, onDone){
  ...
  // sequência de flashes tipo raio/estroboscópio, crescendo até o golpe final
...
function updateDramaticCheck(){
  if(finisherActive) return; // a cinemática do golpe final já controla a câmera sozinha
  if(!battleActive){
