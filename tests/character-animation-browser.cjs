...
(async()=>{
  ...
  try{
    const character=process.argv[2]||'kael';await page.goto(`http://127.0.0.1:${port}/${character}-animation-preview.html`);
    await page.waitForFunction(()=>window.CharacterLab?.ready,null,{timeout:30000});
    const expected=['Idle','Walk','Attack_A','Attack_B','Attack_C','Hit','Death','Special'];
    ...
    if(character==='kael')assert.equal(await page.evaluate(()=>CharacterLab.debug().frenzy),true);
